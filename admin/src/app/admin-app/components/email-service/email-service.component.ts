...
export class EmailServiceComponent implements OnInit {
    ...
    saveEmailService() {
        ...
        if (this.validateForm() && this.emailServiceForm.valid) {
            if (!this.emailServiceId) {
                this.isPost = true;
                this._objEmailService.saveEmailService(this.emailServiceForm.value)
                    .subscribe(res => this.resStatusMessage(res),
                        error => this.errorMessage(error));
            }
            else {
                this.isPost = false;
                this._objEmailService.updateEmailService(this.emailServiceForm.value, this.emailServiceId)
                    .subscribe(res => this.resStatusMessage(res),
                        error => this.errorMessage(error));
            }
        }
    ...
    validateForm() {
        if ((this.emailServiceForm.value.api_Key != "" && typeof this.emailServiceForm.value.api_Key != "undefined") || (this.emailServiceForm.value.host != "" && typeof this.emailServiceForm.value.host != "undefined"))
            return true;
        else {
    ...
    errorMessage(res:any) {
        ...
        for(let i in res.message){
            if(i+1 < res.message.length)
                errorMessage += res.message[i].msg + " & "; 
            else
                errorMessage += res.message[i].msg
        }
