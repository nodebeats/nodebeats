...
export class NewsEditorComponent implements AfterViewInit,OnInit {
  ...
  deleteImage(imageId) {
    swal({
      ...
      ()=> {
        this._objService.deleteImage(this.objNews.image[0].imageName, this.objNews.image[0].imageProperties.imageExtension, this.objNews.image[0].imageProperties.imagePath)
          .subscribe(res=> {
              this.imageDeleted = true;
              this.objNews.image[0].imageName = "";
              this.fileName = "";
              this.drawImageToCanvas(Config.DefaultImage);
