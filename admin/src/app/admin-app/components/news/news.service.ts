...
export class NewsService {
    ...
    updateNewsCategory(objNewsCat:NewsCategoryModel) {
        let body = JSON.stringify(objNewsCat);
        return this._http.put(API_URL + this.NewsCategoryApiRoute + "/" + objNewsCat._id, body)
            .map(res => res.json())
            .catch(this.handleError);
    }
    ...
    updateNews(objNews:NewsModel, file:File, imageDeleted:boolean):Observable<any> {
        return Observable.create(observer => {
            let formData:FormData = new FormData(),
                xhr:XMLHttpRequest = new XMLHttpRequest();
            ...
            xhr.open('PUT', API_URL + this.newsApiRoute + "/" + objNews._id + "?imagedeleted=" + imageDeleted, true);
            xhr.setRequestHeader("Authorization", Config.AuthToken);
            xhr.send(formData);
        });
    }
    ...
    updateNewsImage(newsId:string, objUpdate:NewsImageModel, file:File, imageDeleted:boolean):Observable<any> {
        return Observable.create(observer => {
            let formData:FormData = new FormData(),
                xhr:XMLHttpRequest = new XMLHttpRequest();
            ...
            xhr.open('PUT', API_URL + this.newsImageApi + "/" + newsId + "/" + objUpdate._id + "?imagedeleted=" + imageDeleted, true);
            xhr.setRequestHeader("Authorization", Config.AuthToken);
            xhr.send(formData);
        });
    }
