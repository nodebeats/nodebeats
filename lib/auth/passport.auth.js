...
(function(){
    ...
    module.exports = function(){
                ...
                _p.handleUserLoginLog(req, userID, loginAttempt, failedLogin, done)
                    ...
                    .then(function () {
                        ...
                        if (!user.twoFactorAuthEnabled) {
                            loginStatusMessage = jwtTokenGeneratorHelper.generateJWTToken(req, user);
                        } else {
                            loginStatusMessage = {
            ...
            return new Promise(function(resolve, reject) {
                ...
                _p.handleUserLoginLog(req, userID, loginAttempt, failedLogin, done)
                    .then(function () {
                        ...
                        if (failedLoginAttempt > maxLoginAttempt) {
                            ...
                            if (failedLoginAttempt >= 2) {
                                switch (failedLoginAttempt) {
                                    ...
                                    case 4:
                                        ...
                                        break;
                                    case 5:
                                        expiryDate.setMinutes(expiryDate.getMinutes() + 1440);//block for one day
                                        break;
                                }
                                message = messageConfig.login.ipBlocked;
                                return [message, ipBlockerController.blockLoginIpAddress(ip.address(), expiryDate)];
        ...
        _p.handleUserLoginLog = function(req, userID, loginAttempt, failedLogin, next){
            req.userAgentDetail = userAgent.lookup(req.headers['user-agent']);
            var _ipAddress = ip.address();
            var geo = geoIp.lookup(_ipAddress);
            req.userAgentDetail.ipAddress = _ipAddress;
