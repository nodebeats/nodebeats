                ...
                .then(function(tokenInfo){
                    if(tokenInfo){
                        ...
                        if(!tokenInfo.expired){
                           ...
                           return  [returnObj, _p.updateRegistrationConfirmationToken(tokenInfo.token, tokenInfo.userID)];
                        }
                        else{
                            returnObj = {
                                expired : true
            ...
            return new Promise(function(resolve, reject) {
                hasher.generateRandomBytes(36)
                    ...
                    .spread(function (tokenBytes, emailTemplate) {
                        var url = req.protocol + '://' + req.headers.host + '/api/password-change/confirm/' + tokenBytes;
                        var messageBody = '';
                        if (emailTemplate && emailTemplate.templateBody) {
                            if (emailTemplate.templateBody.indexOf("{{ passwordChange.message }}") > -1) {
                                messageBody = emailTemplate.templateBody.replace('{{ passwordChange.message }}', '<a href=\'' + url + '\'>' + url + '</a>');

                            } else {
                                messageBody = '<div class="container"><h2>Email to confirm your Password Change Request.</h2><p>This email is sent to you to confirm your request for changing password.If you did not request for password change,please ignore this email.</p><br/><p>Please click below link to confirm your password change action:</p><br /><br /><p><a href=\'' + url + '\'>' + url + '</a></p></div>';
                            }
