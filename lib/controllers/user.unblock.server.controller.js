        ...
        return new Promise(function (resolve, reject) {
            hasher.generateRandomBytes(36)
                ...
                .spread(function (tokenBytes, emailTemplate) {
                    var url = req.protocol + '://' + req.headers.host + '/api/unblock/user/' + tokenBytes;
                    var messageBody = '';

                    if (emailTemplate && emailTemplate.templateBody) {
                        if (emailTemplate.templateBody.indexOf("{{ userUnBlock.message }}") > -1) {
                            messageBody = emailTemplate.templateBody.replace('{{ userUnBlock.message }}', '<a href=\'' + url + '\'>' + url + '</a>');

                        } else {
                            messageBody = '<div class="container"><h2>Email to UnBlock your blocked account.</h2><p>This email is sent to you to unblock your login account.If you did not request for Unblocking your account,please ignore this email.</p><br/><p>Please click below link to unblock your account:</p><br /><br /><p><a href=\'' + url + '\'>' + url + '</a></p></div>';
                        }
                        var mailOptions = {
