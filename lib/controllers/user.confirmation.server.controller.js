        ...
        return new Promise(function (resolve, reject) {
            hasher.generateRandomBytes(36)
                ...
                .spread(function (tokenBytes, emailTemplate) {
                    var url = req.protocol + '://' + req.headers.host + '/api/confirm/user/' + tokenBytes;
                    var messageBody = '';

                    if (emailTemplate && emailTemplate.templateBody) {
                        if (emailTemplate.templateBody.indexOf("{{ userRegistrationConfirm.message }}") > -1) {
                            messageBody = emailTemplate.templateBody.replace('{{ userRegistrationConfirm.message }}', '<a href=\'' + url + '\'>' + url + '</a>');

                        } else {
                            messageBody = '<div class="container"><h2>Email to confirm your registration.</h2><p>This email is sent to you to confirm your registration.If you did not request for registration,please ignore this email.</p><br/><p>Please click below link to confirm your registration:</p><br /><br /><p><a href=\'' + url + '\'>' + url + '</a></p></div>';
                        }
