...
var blogController = function(){
    ...
    _p.updateBlogDocumentInfo = function (req,res, next) {
        ...
        if(documentInfo._documentName  && req.body.documentTitle !== ""){
            ...
            dataProviderHelper.updateEmbeddedDocument(Blog, query, newBlogDocument)
                .then(function(){
                    res.status(HTTPStatus.OK);
                    res.json({
            ...
            dataProviderHelper.getAllWithoutFieldsPaginationMultiPopulation(Blog, query, documentFieldsBlog, 'tags', 'seoEntry', populationFieldsTag, populationQueryOptsTag, populationFieldsSeoMetaTag)
                .then(function(blogList){
                    ...
                    if(blogList.length > 0){
                        ...
                        return [blogDetailObj, dataProviderHelper.getEmbeddedDocumentsWithoutPagination(Blog, queryDocOpts, documentFieldsBlogDoc, '$relatedFiles', sortDocOpts, groupOpts), _p.updateBlogArticleViewCount(blogDetailObj._id, updatedCount) ];
                    }else{
                        resolve(null);
                    }
                })
                .spread(function(blogDetailObj, lstBlogDocument){
                    var returnObj = {};
                    var docArr= [];
