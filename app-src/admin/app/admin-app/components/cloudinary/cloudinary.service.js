...
var CloudinaryService = (function () {
    function CloudinaryService(_http) {
        this._http = _http;
        this.apiRoute = "cloudinary";
    }
    CloudinaryService.prototype.saveCloudinarySettings = function (objSave) {
        var body = JSON.stringify(objSave);
        return this._http.post(env_config_1.API_URL + this.apiRoute, body)
            .map(function (res) { return res.json(); })
            .catch(this.handleError);
    };
    CloudinaryService.prototype.updateCloudinarySettings = function (objUpdate) {
        var body = JSON.stringify(objUpdate);
        return this._http.put(env_config_1.API_URL + this.apiRoute + "/" + objUpdate._id, body)
            .map(function (res) { return res.json(); })
            .catch(this.handleError);
    };
    CloudinaryService.prototype.getCloudinarySettings = function () {
        return this._http.get(env_config_1.API_URL + this.apiRoute)
            .map(function (res) { return res.json(); })
            .catch(this.handleError);
    };
    CloudinaryService.prototype.handleError = function (error) {
